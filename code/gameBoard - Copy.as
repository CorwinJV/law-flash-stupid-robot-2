package code 
{
	...
	//import code.Oswitch;
	//import code.Oteleport;
	//import code.subroutine;
	...
	{		
		...
		public function execute(e:MouseEvent)
		{
			...
			trace("updated/processing commands as if execute were fired");
			populateMapImages();
			//processRobot();
		}
...
//
		...
		public function processKeyboard()
		{
			...
			else if (Key.isDown(51))	// rotate robot left	3
			{
				myRobot.rotate( -1);
				setRobotImage(myRobot.getDirection());
				reInjectRobotImage();
				trace("myRobot.rotate(-1)");
			}
			else if (Key.isDown(52))	// rotate robot right	4
			{
				myRobot.rotate(1);
				setRobotImage(myRobot.getDirection());
				reInjectRobotImage();
		...
		public function processRobot()
		{
			...
			case AiInstructionsEnum.MOVE_FORWARD1.toInt():
				...
				if(mapList[robotX][robotY].getType() == tileEnums.TIce)
				{
...
//							timer = clock();
		...
		public function interfaceHasFiredExecuteOrder(e:Event)
		{
			...
			{
				SUB1.addCommand(exListSub1[i]);
			}
			
			SUB2.clearInstructions();
			for (var i:int = 0; i < exListSub2.length; i++)
			{
				SUB2.addCommand(exListSub2[i]);
			}		
			...
			for (var i:int = 0; i < exList.length; i++)
			{
		...
		public function processExecutionCycle(e:TimerEvent)
		{
			...
			{
				executionCycleTimer = new Timer(executionCycleInterval, 1);
				executionCycleTimer.addEventListener(TimerEvent.TIMER, processExecutionCycle);
				executionCycleTimer.start();
			}
		}
		...
		{	
			...
			mapOffsetY = (screenHeight / 2) - (hh * currentY/2) - (hh * currentX/2) + hh;
