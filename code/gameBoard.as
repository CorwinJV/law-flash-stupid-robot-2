...
//
		...
		public function loadMapFromFile(filename:String)
		{
			...
			var levelToLoadName:Class = getDefinitionByName(filename) as Class;
			
			var loadedLevel:MovieClip = (new levelToLoadName() as MovieClip);
			//var testLevel:MovieClip = new testMap1orig();
			
			//tempString = textLoader.data;
			tempString = loadedLevel.text.text;
			
			//for (x = 0; x < tempString.length; x++)
			//{
				//trace ("Char ", x, " = ", tempString.substr(x, 1), " in code is", tempString.charCodeAt(x), " which translates to ", tempString.charCodeAt(x) - 48);
			//}
			
			
			////////////
			// this whole next section is the load function and its going to be disabled rather than commented out to make my life easier
			...
			for (y = 0; y < nHeight; y++)
			{
				...
				for (x = 0; x < nWidth; x++)
				{
					// read in a value
					tens = (tempString.charCodeAt(fileIndex++) - 48) * 10; 	ones = tempString.charCodeAt(fileIndex++) - 48; fileIndex++;
					//trace (" read in tens of ", tens, " and ones of ", ones);
					var val:int = tens + ones;
					if ((val == tileEnums.THalfBottomL.toInt()) || (val == tileEnums.THalfBottomR.toInt()))
					{
						val = tileEnums.TGap.toInt();
			...
			if (1 == 0)
			{
			// this curly brace here goes with the disabling crap to make my life easier
			}
			/////
			...
			else
			{
				switch(robotDirection)
				{
					...
					case 2: // bottom left
						...
						break;
					case 3: // top left
						myGameVar.robotJumpSuccessTL = true;
						break;
				}
			}
